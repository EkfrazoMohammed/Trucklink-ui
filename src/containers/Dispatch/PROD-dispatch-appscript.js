...
function fetchAndStoreHubs() {
  var sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("dispatch_challans"); // Change to your sheet name if different
  const hello =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE3MjAxNzg1NjAsImV4cCI6MTcyMjc3MDU2MCwiaXNzIjoiaHV0ZWNoc29sdXRpb25zLmNvbSIsInN1YiI6ImVtYWlsPWRocnV2YUB0cnVja2xpbmsuY29tcm9sZT1BZG1pbiIsInJvbGUiOiJBZG1pbiIsImVtYWlsIjoiZGhydXZhQHRydWNrbGluay5jb20ifQ.53RLO9Aya9IxHDSSRxgVw66ZZH6VSiU341g3g-7RixU";
  const headersOb = {
    headers: {
  ...
  if (data.message === "successfully fetched hub data") {
    ...
    var hiddenSheet =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName("all_hubs");
    if (!hiddenSheet) {
      hiddenSheet =
        SpreadsheetApp.getActiveSpreadsheet().insertSheet("all_hubs");
    }
    hiddenSheet.clear();
...
function setupDropdowns(hiddenSheet) {
  var sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("dispatch_challans");
  if (!sheet) {
...
function fetchmaterialType() {
  ...
  for (var row = 2; row <= lastRow; row++) {
    ...
    var materialUrl = `https://trucklink.in/prod/v1/get-material/${hubId}`;
    ...
    if (materialData.message === "successfully fetched material data") {
      ...
      if (materialTypes.length > 0) {
        ...
        hiddenSheet.clear();
        hiddenSheet
          .getRange(1, 1, materialTypeNames.length, 1)
          .setValues(materialTypeNames); // Column A for location names
...
function setupmaterialTypeDropdowns(hiddenSheet, rowIndex) {
  ...
  var materialTypeNamesRange = hiddenSheet.getRange(
    "A1:A" + hiddenSheet.getLastRow()
  );
  var materialTypeNames = materialTypeNamesRange
    .getValues()
    .flat()
  ...
  var rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(materialTypeNames)
    .build();
  validationRange.setDataValidation(rule);
}
...
function fetchLoadLocations() {
  ...
  for (var row = 2; row <= lastRow; row++) {
    ...
    var locationUrl = `https://trucklink.in/prod/v1/get-load-location/${hubId}`;
    ...
    if (locationData.message === "successfully fetched location data") {
      ...
      var hiddenSheet =
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName(hiddenSheetName);
      if (!hiddenSheet) {
        hiddenSheet =
          SpreadsheetApp.getActiveSpreadsheet().insertSheet(hiddenSheetName);
        SpreadsheetApp.flush();
      }
      hiddenSheet.clear();
      hiddenSheet
        .getRange(1, 1, locationNames.length, 1)
        .setValues(locationNames);
...
function setupLocationDropdowns(hiddenSheet, rowIndex) {
  ...
  var locationNamesRange = hiddenSheet.getRange(
    "A1:A" + hiddenSheet.getLastRow()
  );
  ...
  var rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(locationNames)
    .build();
  validationRange.setDataValidation(rule);
}
...
function fetchdeliveryLocations() {
  ...
  for (var row = 2; row <= lastRow; row++) {
    ...
    var locationUrl = `https://trucklink.in/prod/v1/get-delivery-location/${hubId}`;
...
function fetchVehicleDetails() {
  ...
  for (var row = 2; row <= lastRow; row++) {
    ...
    if (hubId) {
      // Proceed only if hubId is not empty
      var url = `https://trucklink.in/prod/v1/get-vehicle-details?page=1&limit=1000&hubId=${hubId}`;
      ...
      if (data.message === "Successfully retrived users informations") {
        ...
        hiddenSheet.clear();
        hiddenSheet
          .getRange(1, 1, vehicleNames.length, 1)
          .setValues(vehicleNames); // Column A for vehicle names
        hiddenSheet.getRange(1, 2, vehicleIds.length, 1).setValues(vehicleIds); // Column B for vehicle IDs
        hiddenSheet
          .getRange(1, 3, vehicleBanks.length, 1)
          .setValues(vehicleBanks); // Column C for vehicleBanks
        ...
        hiddenSheet
          .getRange(1, 7, ownerPhones.length, 1)
          .setValues(ownerPhones); // Column G for ownerPhones

        // Set up dropdowns and auto-fill vehicle IDs and truck types
        setupVehicleDropdowns(hiddenSheet, row);
      } else {
        Logger.log(
          `Error fetching vehicle details for hubId ${hubId}: ${data.message}`
        );
      }
    } else {
...
function setupVehicleDropdowns(hiddenSheet, rowIndex) {
  ...
  var vehicleNamesRange = hiddenSheet.getRange(
    "A1:A" + hiddenSheet.getLastRow()
  );
  ...
  var rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(vehicleNames)
    .build();
  validationRange.setDataValidation(rule);

  var vehicleData = hiddenSheet
    .getRange("A1:Z" + hiddenSheet.getLastRow())
    .getValues(); // Adjust range for vehicle data
